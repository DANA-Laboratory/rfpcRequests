        ...
        element.bind("keyup", function (event) {
            if (scope.filtertext.length > 2){
                ...
                for (var item in scope.tasks) {
                    if (scope.tasks[item].name.search(scope.filtertext) !== -1) {
                        scope.alltasks.push(scope.tasks[item]);
                    }
                }
            } else {
...
itRequestApp.config(['$routeProvider',
  function ($routeProvider) {
    $routeProvider.when('/RemoveItem/:id', {
        templateUrl: 'panelSuccessRemove',
        controller: 'RemoveItemController',
    });
    $routeProvider.when('/AddNewItem/:id', {
